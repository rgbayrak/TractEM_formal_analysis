...
exDir = {'/nfs/masi/bayrakrg/tractem_data/corrected'};
...
for t = 1:length(exDir)
    ...
    subjectDir = fullfile(exDir{t}, 'BLSA18');  % directory names are as follows -> subject_rater
        ...
        for r = 1:length(regionList)
            ...
            if ~isempty(densityDir)
                if length(strfind(densityDir(1).name,'_')) == 1
                    ...
                    [diceMatrix, same_regionS] = region_dice(densityDir, density, threshold, d);
                ...
                elseif length(strfind(densityDir(1).name,'_')) == 2
                    ...
                    if length(densityLDir) > 1
                        if length(strfind(densityLDir(1).name,'_L')) == 1
                            ...
                            [diceL, same_regionL] = region_dice(densityLDir, densityL, threshold, dl);
        ...
        %                     pause
                    ...
                    if length(densityRDir) > 1
                        if length(strfind(densityRDir(2).name,'_R')) == 1
                            ...
                            [diceR, same_regionR] = region_dice(densityRDir, densityR, threshold, dr);
...
%%
save('same_BLSA18_region.mat', 'region_data')
