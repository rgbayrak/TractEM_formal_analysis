...
% HCP
exDir              = '/home-local/bayrakrg/Dropbox*VUMC*/complete_HCP_*';
subjectDir = fullfile(exDir, '878877_*');  % directory names are as follows -> subject_rater
subDir = fullfile(subjectDir, '*');  % tract names are defined as published

mat_exDir = '/home-local/bayrakrg/Dropbox*VUMC*/tractEM/HCP_auto';
matDir = fullfile(mat_exDir, '878877_DICE_mat');  % directory names are as follows -> subject_rater
dice_mat_files = dir(fullfile([matDir, '/*dice.mat']));
name_mat_files = dir(fullfile([matDir, '/*nameMe.mat']));

background = '/home/local/VANDERBILT/bayrakrg/Desktop/878877_auto/878877_gqi_fib_qa.nii.gz';
                ...
                for j = i:d
                    ...
                    if diceMatrix(i,j) < 1
                        if i ~= j && convertCharsToStrings(dirParti(1:4)) == convertCharsToStrings(dirPartj(1:4))
                            ...
                            saveas(f,['/home-local/bayrakrg/Dropbox (VUMC)/tractEM/HCP_auto/overlay/' dirParti '_vs_' dirPartj '_' name '_overlay.jpg']); 
                            close all;
                ...
                for j = i:dl
                    ...
                    if diceL(i,j) < 1
                        if i ~= j && convertCharsToStrings(dirParti(1:4)) == convertCharsToStrings(dirPartj(1:4))
                            ...
                            name = densityDir(1).name(1:end-15);
                            saveas(g,['/home-local/bayrakrg/Dropbox (VUMC)/tractEM/HCP_auto/overlay/' dirParti '_vs_' dirPartj '_' name '_overlay.jpg']); 
                            close all;
                ...
                for j = i:dr
                    ...
                    if diceR(i,j) < 1
                        if i ~= j && convertCharsToStrings(dirParti(1:4)) == convertCharsToStrings(dirPartj(1:4))
                            ...
                            name = densityDir(1).name(1:end-15);
                            saveas(h,['/home-local/bayrakrg/Dropbox (VUMC)/tractEM/HCP_auto/overlay/' dirParti '_vs_' dirPartj '_' name '_overlay.jpg']); 
                            close all;
