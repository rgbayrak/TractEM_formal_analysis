...
exDir = '/home/local/VANDERBILT/bayrakrg/Dropbox*VUMC*/complete_HCP_subjects'; % only the dir address
subjectDir = dir(['/home/local/VANDERBILT/bayrakrg/Dropbox*VUMC*/complete_HCP_subjects', '/*']); % the dir with the subfolders
disp('Dropbox pulled data errors')
...
for i = 1:length(subjectDir)
    trackFolder = dir(fullfile(exDir, subjectDir(i).name));
        ...
        if trackFolder(j).isdir && ...
            ...
            if abb_pos <= length(abbList)
                for q = 1:length(fileDir_dens)
                    ...
                    if length(name(1:underscore_pos(1)-1)) ~= length(abbList{abb_pos}) | name(1:underscore_pos(1)-1) ~= abbList{abb_pos}
            ...
            for d = 1:length(fileDir_ROA)
                ...
                disp(['For: ' fullfile(subjectDir(i).name,trackFolder(j).name,fileDir_ROA(d).name), 'no ROA files found!'])
                end
            end
            ...
            for d = 1:length(fileDir_seed)
                ...
                disp(['For: ' fullfile(subjectDir(i).name,trackFolder(j).name,fileDir_seed(d).name), ' no seed files are found!'])
                end
            end
