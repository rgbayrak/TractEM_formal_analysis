...
% Load the data
BLSA = load('reproducibility/BLSA_Apr_rMSE.mat');
...
% BLSA_table = struct2table(BLSA.all_rMSE);
% BLSA_auto_table = struct2table(BLSA_auto.all_rMSE);

% %% Tract Reproducibility Analysis
% 
mask_BLSA_tract = unique({BLSA.all_rMSE.tract});
% mask_BLSA_auto_tract = unique({BLSA_auto.all_rMSE.tract});
% 
BLSA_tract = [];
for t = 1:length(mask_BLSA_tract)
    mask_per_tract = strcmp({BLSA.all_rMSE.tract}, mask_BLSA_tract(t));
    tmp_tract = BLSA.all_rMSE(mask_per_tract);
    
    tmp_BLSA_rMSE = [];
    for d = 1:length(tmp_tract)
        tmp_BLSA_rMSE(d, 1) = tmp_tract(d).rMSE;
    end
  
    if size(BLSA_tract,1) > size(tmp_BLSA_rMSE,1)
        tmp_BLSA_rMSE = [tmp_BLSA_rMSE; nan(size(BLSA_tract,1) - size(tmp_BLSA_rMSE,1), 1)];
    else
        BLSA_tract= [BLSA_tract; nan(size(tmp_BLSA_rMSE,1) - size(BLSA_tract,1), size(BLSA_tract,2))];
    end
    BLSA_tract = [BLSA_tract tmp_BLSA_rMSE];
end
% % 
% BLSA_auto_tract = [];
% for t = 1:length(mask_BLSA_auto_tract)
%     mask_per_tract = strcmp({BLSA_auto.all_rMSE.tract}, mask_BLSA_auto_tract(t));
%     tmp_tract = BLSA_auto.all_rMSE(mask_per_tract);
%     
%     tmp_BLSA_rMSE = [];
%     for d = 1:length(tmp_tract)
%         tmp_BLSA_rMSE(d, 1) = tmp_tract(d).rMSE;
%     end
%   
%     if size(BLSA_auto_tract,1) > size(tmp_BLSA_rMSE,1)
%         tmp_BLSA_rMSE = [tmp_BLSA_rMSE; nan(size(BLSA_auto_tract,1) - size(tmp_BLSA_rMSE,1), 1)];
%     else
%         BLSA_auto_tract= [BLSA_auto_tract; nan(size(tmp_BLSA_rMSE,1) - size(BLSA_auto_tract,1), size(BLSA_auto_tract,2))];
%     end
%     BLSA_auto_tract = [BLSA_auto_tract tmp_BLSA_rMSE];
% end
% 
...
% % 
mask_BLSA_rater = unique({BLSA.all_rMSE.rater1 BLSA.all_rMSE.rater2});
% mask_BLSA_auto_rater = unique({BLSA_auto.all_rMSE.rater1 BLSA_auto.all_rMSE.rater2});
% %
% % 
% To exclude some raters
list_BLSA_rater = unique({BLSA.all_rMSE.rater1 BLSA.all_rMSE.rater2});
...
for t = 1:length(mask_BLSA_rater)
    ...
    mask_per_rater1 = strcmp({BLSA.all_rMSE.rater1}, mask_BLSA_rater(t));
    mask_per_rater2 = strcmp({BLSA.all_rMSE.rater2}, mask_BLSA_rater(t));
    
    tmp_rater1 = BLSA.all_rMSE(mask_per_rater1);
    tmp_rater2 = BLSA.all_rMSE(mask_per_rater2);
...
%     tmp_rater2 = tmp_2(tmp_mask_2);
    ...
    for g = 1:length(tmp_rater1)
        tmp_BLSA_rMSE1(g, 1) = tmp_rater1(g).rMSE;
    end
    
    tmp_BLSA_rMSE2 = [];
    for h = 1:length(tmp_rater2)
        tmp_BLSA_rMSE2(h, 1) = tmp_rater2(h).rMSE;
    end
    
    tmp_BLSA_rMSE = [];
    tmp_BLSA_rMSE = [tmp_BLSA_rMSE1; tmp_BLSA_rMSE2];
    
    % padding with NaN
    if size(tmp_BLSA_rMSE,1) > size(BLSA_rater,1)
        BLSA_rater= [BLSA_rater; nan(size(tmp_BLSA_rMSE,1) - size(BLSA_rater,1), size(BLSA_rater,2))]; % adding rows
    else
        tmp_BLSA_rMSE = [tmp_BLSA_rMSE; nan(size(BLSA_rater,1) - size(tmp_BLSA_rMSE,1), 1)]; % adding rows
    end
    BLSA_rater = [BLSA_rater tmp_BLSA_rMSE]; % adding columns
    
end


% % To exclude some raters
% list_BLSA_auto_rater = unique({BLSA_auto.all_rMSE.rater1 BLSA_auto.all_rMSE.rater2});
...
%     
%     mask_per_rater1 = strcmp({BLSA_auto.all_rMSE.rater1}, mask_BLSA_auto_rater(t));
%     mask_per_rater2 = strcmp({BLSA_auto.all_rMSE.rater2}, mask_BLSA_auto_rater(t));
%     
%     tmp_rater1 = BLSA_auto.all_rMSE(mask_per_rater1);
%     tmp_rater2 = BLSA_auto.all_rMSE(mask_per_rater2);
%     
...
%     
%     % padding with NaN
%     if size(tmp_BLSA_rMSE,1) > size(BLSA_auto_rater,1)
%         BLSA_auto_rater= [BLSA_auto_rater; nan(size(tmp_BLSA_rMSE,1) - size(BLSA_auto_rater,1), size(BLSA_auto_rater,2))]; % padding with rows
%     else
%         tmp_BLSA_rMSE = [tmp_BLSA_rMSE; nan(size(BLSA_auto_rater,1) - size(tmp_BLSA_rMSE,1), 1)]; % padding with rows
%     end
%     BLSA_auto_rater = [BLSA_auto_rater tmp_BLSA_rMSE]; % adding columns
%     
% end
...
% diff_mean = nanmean(rMSE_ar_rr, 1);
% [~, sort_diff_idx] = sort(rMSE_ar_rr,'descend');
% diff_sorted_tract = nan(size(rMSE_ar_rr));
% for i=1:size(rMSE_ar_rr, 2)
% diff_sorted_tract(i) = rMSE_ar_rr(sort_diff_idx(i));
...
% sort the intra-subject tract rMSE 
...
% % % sorting auto tracts
% auto_mean = nanmean(BLSA_auto_tract, 1);
% [~, sort_auto_idx] = sort(auto_mean,'descend');
% auto_sorted_tract = zeros(size(BLSA_auto_tract));
% BLSA_auto_tract(isnan(BLSA_auto_tract)) = 0;
% for i=1:size(BLSA_auto_tract, 2)
%     auto_sorted_tract(:,i) = BLSA_auto_tract(:,sort_auto_idx(i));
...
% BLSA = load('reproducibility/BLSA_auto_crMSE.mat');
% 
% % To exclude some raters
% list_BLSA_rater = unique({BLSA.all_rMSE.rater1 BLSA.all_rMSE.rater2});
% mask_BLSA_rater = zeros(1,length(list_BLSA_rater));
...
% mask_BLSA_rater = list_BLSA_rater(logical(mask_BLSA_rater));
% 
% list_BLSA_tract = unique({BLSA.all_rMSE.tract});
...
%     mask_per_tract = strcmp({BLSA.all_rMSE.tract}, list_BLSA_tract(t));
%     tmp_tract = BLSA.all_rMSE(mask_per_tract);
%     BLSA_rater = [];
