function [nameMe, temp_hd, temp_hdmean, temp_modHausdd, temp_hd90] = mhd_th(tractDir, tract, threshold, d)
    ...
    %                 mhd = bwModHausDist(im1, im2);
                    ...
                    % Initialization
                    if ~init
                        temp_hd = Inf([length(tractDir) length(tractDir)]);
                        temp_hdmean = Inf([length(tractDir) length(tractDir)]);
                        temp_modHausdd = Inf([length(tractDir) length(tractDir)]);
                        temp_hd90 = Inf([length(tractDir) length(tractDir)]);
                        init = 1;
                    end
