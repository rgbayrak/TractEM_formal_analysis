...
hd_mat_files = dir(fullfile([matDir, '/*seed1_temp_hdR.mat']));
hdmean_mat_files = dir(fullfile([matDir, '/*seed1_temp_hdmeanR.mat']));
modHausdd_mat_files = dir(fullfile([matDir, '/*seed1_temp_modHausddR.mat']));
hd90_mat_files = dir(fullfile([matDir, '/*seed1_temp_hd90R.mat']));
name_mat_files = dir(fullfile([matDir, '/*seed1_nameMe.mat']));

icc_mat_filesR = dir(fullfile([rep_matDir, '/unc_R_icc.mat']));
...
icc_matR = load(fullfile(icc_mat_filesR(l).folder, icc_mat_filesR(l).name));
...
for d = 1:length(uni_id)
    ...
    if sum(mask_id(:)) > 1
        icc_mat_temp = struct2cell(icc_matR); 
...
ylabel('ICC');
ylim([0 1])
title('unc R seed label')


% save('/share4/bayrakrg/tractEM/postprocessing/metric_analysis/label/mhd/icc_seed1_R_mhd.mat', 'icc_mat_intra_subject')
