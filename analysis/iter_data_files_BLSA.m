...
exDir = '/home-local/bayrakrg/Dropbox*VUMC*/19_new_BLSA'; % only the dir address
subjectDir = dir(['/home-local/bayrakrg/Dropbox*VUMC*/19_new_BLSA', '/*']); % the dir with the subfolders
...
%     abb_pos = 1;
    ...
    for j = 1:length(trackFolder)
        if trackFolder(j).isdir && ...
            ...
            fileDir_dens = dir(fullfile(exDir, subjectDir(i).name, trackFolder(j).name, '*_density.nii.gz'));
            if length(fileDir_dens) == 0
               disp(['For: ' fullfile(subjectDir(i).name,trackFolder(j).name) ' no density files found!'])
            end
            fileDir_tract = dir(fullfile(exDir, subjectDir(i).name, trackFolder(j).name, '*_tract.trk.gz'));
            if length(fileDir_tract) == 0
               disp(['For: ' fullfile(subjectDir(i).name,trackFolder(j).name) ' no tract files found!'])
            end
...
%                         disp(abbList{abb_pos})
                        ...
                        if length(strfind(fileDir_dens.name,'_')) ~= 1
                            disp(['For: ' fullfile(subjectDir(i).name,trackFolder(j).name,fileDir_dens.name), ' a single density file found! File mislabeled or missing!'])
                        end
...
%                         disp(abbList{abb_pos})
                        ...
                        if length(strfind(fileDir_dens(1).name,'_')) ~= 2
                            disp(['For: ' fullfile(subjectDir(i).name,trackFolder(j).name,fileDir_dens(1).name), ' two files found but something is not right!'])
                        end


                        % Make sure second file has two underscores
                        if length(strfind(fileDir_dens(2).name,'_')) ~= 2
                            disp(['For: ' fullfile(subjectDir(i).name,trackFolder(j).name,fileDir_dens(2).name), ' two files found but something is not right!'])
                        end
                    else
                        disp(['For: ' fullfile(subjectDir(i).name,trackFolder(j).name) ' not the right number of density!'])
                    end  
                end
...
%                     end
                    ...
                    if length(fileDir_tract) == 1
                        ...
                        if length(strfind(fileDir_tract.name,'_')) ~= 1
                            disp(['For: ' fullfile(subjectDir(i).name,trackFolder(j).name,fileDir_tract.name), ' only a single tract file found! File mislabeled or missing!'])
                        end

                    elseif length(fileDir_tract) == 2
                        % Make sure first file has two underscores
                        if length(strfind(fileDir_tract(1).name,'_')) ~= 2
                            disp(['For: ' fullfile(subjectDir(i).name,trackFolder(j).name,fileDir_tract(1).name), ' two tract files found but something is not right!'])
                        end

                        % Make sure second file has two underscores
                        if length(strfind(fileDir_tract(2).name,'_')) ~= 2
                            disp(['For: ' fullfile(subjectDir(i).name,trackFolder(j).name,fileDir_tract(2).name), ' two tract files found but something is not right!'])
                        end
                    else
                        disp(['For: ' fullfile(subjectDir(i).name,trackFolder(j).name) ' not the right number of tract files!'])
                    end
                end
%                 abb_pos = abb_pos+1;
%             end
            ...
            % ROA and seed check
            fileDir_ROA = dir(fullfile(exDir, subjectDir(i).name, trackFolder(j).name, '*ROA*.nii.gz'));
            c_ROA = c_ROA + 1;
            ROA = [ROA fullfile(trackFolder(j).name, fileDir_ROA(1).name)];
            if isempty(fileDir_ROA)
            disp(['For: ' fullfile(subjectDir(i).name,trackFolder(j).name,fileDir_ROA.name), 'no ROA files found!'])
            end
            
            fileDir_seed = dir(fullfile(exDir, subjectDir(i).name, trackFolder(j).name, '*seed*.nii.gz'));
            c_seed = c_seed + 1;
            if isempty(fileDir_seed)
            disp(['For: ' fullfile(subjectDir(i).name,trackFolder(j).name,fileDir_seed.name), ' no seed files are found!'])
            end
            
            
            % MISLABELED TRACT & DENSITY---------------------------------------------------------------------------------------------------------------------------
            % Checks for the mislabeled files
            fileDir_density_trk = dir(fullfile(exDir, subjectDir(i).name, trackFolder(j).name, '*_density.trk.gz'));
            fileDir_track_trk = dir(fullfile(exDir, subjectDir(i).name, trackFolder(j).name, '*_track.trk.gz'));

            % density.trk
            for d = 1:length(fileDir_density_trk)
                disp(['For: ' fullfile(subjectDir(i).name,trackFolder(j).name,fileDir_density_trk(d).name), ' density file saved as trk file, should be nii!'])
            end

            % track.trk
            for t = 1:length(fileDir_track_trk)
                disp(['For: ' fullfile(subjectDir(i).name,trackFolder(j).name,fileDir_track_trk(t).name), ' file saved as track file, should be tract!'])
            end
        end
    end
