...
for l = 1:length(abbList)
    ...
    labelDir = dir(fullfile(subDir, [abbList{l} '_*end1.nii.gz'])); % not every folder has separate label files (left & right)
    ...
    if ~isempty(labelDir)
        if length(strfind(labelDir(1).name,'_')) == 1 && length(labelDir) > 4
            ...
            save(['/share4/bayrakrg/tractEM/postprocessing/metric_analysis/label/mhd/human_mhd_mat/BLSA/' abbList{l} '_end1_hd.mat'], 'hd');
            save(['/share4/bayrakrg/tractEM/postprocessing/metric_analysis/label/mhd/human_mhd_mat/BLSA/' abbList{l} '_end1_hdmean.mat'], 'hdmean');
            save(['/share4/bayrakrg/tractEM/postprocessing/metric_analysis/label/mhd/human_mhd_mat/BLSA/' abbList{l} '_end1_modHausdd.mat'], 'modHausdd');
            save(['/share4/bayrakrg/tractEM/postprocessing/metric_analysis/label/mhd/human_mhd_mat/BLSA/' abbList{l} '_end1_hd90.mat'], 'hd90');
            save(['/share4/bayrakrg/tractEM/postprocessing/metric_analysis/label/mhd/human_mhd_mat/BLSA/' abbList{l} '_end1_nameMe'], 'nameMe');
    %         % plotting
    ...
    %         close all;
        ...
        elseif length(strfind(labelDir(1).name,'_')) == 2 
            labelLDir = dir(fullfile(subDir, [abbList{l} '_L_end1.nii.gz']));
            labelRDir = dir(fullfile(subDir, [abbList{l} '_R_end1.nii.gz']));
  
            if ~isempty(labelLDir)
                if length(strfind(labelLDir(1).name,'_L_')) == 1 && length(labelLDir) > 4
                    labelLDir = dir(fullfile(subDir, [abbList{l} '_L_end1.nii.gz'])); % not every folder has separate label files (left & right)
                    dl = length(labelLDir);
                    labelL = load_only_nii_img(labelLDir);
                    [nameMe, hdL, hdmeanL, modHausddL, hd90L] = mhd(labelLDir, labelL, dl);
                    save(['/share4/bayrakrg/tractEM/postprocessing/metric_analysis/label/mhd/human_mhd_mat/BLSA/' abbList{l} '_L_end1_hdL.mat'], 'hdL');
                    save(['/share4/bayrakrg/tractEM/postprocessing/metric_analysis/label/mhd/human_mhd_mat/BLSA/' abbList{l} '_L_end1_hdmeanL.mat'], 'hdmeanL');
                    save(['/share4/bayrakrg/tractEM/postprocessing/metric_analysis/label/mhd/human_mhd_mat/BLSA/' abbList{l} '_L_end1_modHausddL.mat'], 'modHausddL');
                    save(['/share4/bayrakrg/tractEM/postprocessing/metric_analysis/label/mhd/human_mhd_mat/BLSA/' abbList{l} '_L_end1_hd90L.mat'], 'hd90L');
                    save(['/share4/bayrakrg/tractEM/postprocessing/metric_analysis/label/mhd/human_mhd_mat/BLSA/' abbList{l} '_L_end1_nameMe'], 'nameMe');
        %             % plotting
        %             p = figure('Visible','on');   
        %             imagesc(mhdL)
        ...
        %             close all;
            ...
            if ~isempty(labelRDir)
                if length(strfind(labelRDir(1).name,'_R_')) == 1 && length(labelRDir) > 4
                    labelRDir = dir(fullfile(subDir, [abbList{l} '_R_end1.nii.gz'])); % not every folder has separate label files (left & right)
                    dr = length(labelRDir);
                    labelR = load_only_nii_img(labelRDir);
                    [nameMe, hdR, hdmeanR, modHausddR, hd90R] = mhd(labelRDir, labelR, dr);
                    save(['/share4/bayrakrg/tractEM/postprocessing/metric_analysis/label/mhd/human_mhd_mat/BLSA/' abbList{l} '_R_end1_hdR.mat'], 'hdR');
                    save(['/share4/bayrakrg/tractEM/postprocessing/metric_analysis/label/mhd/human_mhd_mat/BLSA/' abbList{l} '_R_end1_hdmeanR.mat'], 'hdmeanR');
                    save(['/share4/bayrakrg/tractEM/postprocessing/metric_analysis/label/mhd/human_mhd_mat/BLSA/' abbList{l} '_R_end1_modHausddR.mat'], 'modHausddR');
                    save(['/share4/bayrakrg/tractEM/postprocessing/metric_analysis/label/mhd/human_mhd_mat/BLSA/' abbList{l} '_R_end1_hd90R.mat'], 'hd90R');
                    save(['/share4/bayrakrg/tractEM/postprocessing/metric_analysis/label/mhd/human_mhd_mat/BLSA/' abbList{l} '_R_end1_nameMe'], 'nameMe');
        %             % plotting
