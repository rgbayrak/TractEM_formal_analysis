...
D = load('reproducibility/HCP_Nov.mat');
...
figure(1); clf
boxplot(Dinter.dice,Dinter.raters,'whisker',1.5,'orientation','horizontal')
hold on
...
figure(2); clf
boxplot(Dinter.dice,Dinter.tract,'whisker',1.5,'orientation','horizontal')
hold on
plot([median(Dinter.dice) median(Dinter.dice)],get(gca,'YLim'),'k-')
hold off
...
Frater = fitlme(Dinter,'dice ~ Aviral+Bruce+Christa+Eugene+Xuan+Yi+Yufei');
...
xlabel('Dice difference relative to Jasmine')
...
cind = 2:61; ind = cind-1; ylim = [ind(1)-1 ind(end)+1];
...
cind = 62:68; ind = cind-1; ylim = [ind(1)-1 ind(end)+1];
plot(F.Coefficients.Estimate(cind),ind,'o')
set(gca,'YLim',ylim)
plot([0 0],get(gca,'YLim'),':k')
set(gca,'YTick',ind)
set(gca,'YTickLabel',cellfun(@(x) strrep(x,'_',' '),F.Coefficients.Name(cind),'UniformOutput',false))
title('Rater main effects')
