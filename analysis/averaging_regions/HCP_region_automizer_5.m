...
folder = fullfile('/share4/bayrakrg/tractEM/postprocessing/HCP_regions/threshold_9/', tractList);

if ~isfolder(folder)
    mkdir ('/share4/bayrakrg/tractEM/postprocessing/HCP_regions/threshold_9/',  tractList{1})
end
        ...
        if single ~= 0  
            ...
            if axis == 1
                if ~isempty(spec_tract_dir)
                    ...
                    vol.img(single, :, :) = a;
                    sum(vol.img(:))
                    save_nii(vol, ['/share4/bayrakrg/tractEM/postprocessing/HCP_regions/' 'threshold_9' filesep tractList{1} filesep abbList{1} '_' regionList{t} '.nii.gz']);
                end
            end
        ...
        if L ~= 0
            ...
            if axis == 1
                if ~isempty(spec_tract_dir_R)
                    ...
                    volR.img(R, :, :) = c;
                    sum(volR.img(:))
                    save_nii(volR, ['/share4/bayrakrg/tractEM/postprocessing/HCP_regions/' 'threshold_9' filesep tractList{1} filesep abbList{1} '_R_' regionList{t} '.nii.gz']);
                end
                ...
                if ~isempty(spec_tract_dir_L)
                    ...
                    volL.img(L, :, :) = b;
                    sum(volL.img(:))
                    save_nii(volL, ['/share4/bayrakrg/tractEM/postprocessing/HCP_regions/' 'threshold_9' filesep tractList{1} filesep abbList{1} '_L_' regionList{t} '.nii.gz']);
                    clear size;
                end
            end
