...
exDir              = '/home-local/bayrakrg/Dropbox*VUMC*/tractEM/BLSA_auto';
matDir = fullfile(exDir, '7678_DICE_mat');  % directory names are as follows -> subject_rater
                        ...
                        for y = x:sum(mask_id(:))
                            if x ~= y
                                ...
                                if length(strfind(dice_mat_files(l).name,'_')) == 2
                                    all_dice(count).tractAll = dice_mat_files(l).name(1:end-11);
                                else
                                    all_dice(count).tractAll = dice_mat_files(l).name(1:end-9);
                                end
...
save('tractem/analysis/reproducibility/7678_Aug02_01.mat', 'all_dice')
