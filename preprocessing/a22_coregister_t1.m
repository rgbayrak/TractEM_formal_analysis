function [ct1,cseg] = a22_coregister_t1(dwi,bval,t1,seg, out_dir)

% Find and load the b=0, compute b=0 mean image and its COM from DWI
bvals = load(bval);
dind = find(bvals'<10);
...
tgt = fullfile(out_dir,'dwi_b0_forcoreg.nii');
Vout.fname = tgt;
...
matlabbatch{tag}.spm.spatial.coreg.estimate.eoptions = struct( ...
        ...
        'fwhm', [7 7] );
spm_jobman('run',matlabbatch) % takes a couple of mins ...
